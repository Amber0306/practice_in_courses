...
public class PuzzleGameBoard {
    ...
    public PuzzleGameBoard(int rows, int columns) {
        ...
        for(int i=0;i<mRows;i++)
        {
            ...
            {
                mTileGrid[i][j]=new PuzzleGameTile();
            }
        }
    }
    ...
    public boolean isEmptyTile(int row, int col) {
        ...
        if(mTileGrid[row][col].isEmpty()==true) {
            return true;
        }else {
            return false;
        }
    ...
    public boolean areTilesNeighbors(
        ...
        //if(isWithinBounds(firstTileRow,firstTileCol)&&isWithinBounds(secondTileRow,secondTileCol)) {
            if ((firstTileRow == secondTileRow) && (firstTileCol == secondTileCol - 1)) {
                return true;
            }//the second one is in the up
            else if ((firstTileRow == secondTileRow) && (firstTileCol == secondTileCol + 1)) {
                return true;
            }//the second one is in the down
            else if ((firstTileRow == secondTileRow + 1) && (firstTileCol == secondTileCol)) {
                return true;
            }//the second one is in the left
            else if ((firstTileRow == secondTileRow - 1) && (firstTileCol == secondTileCol)) {
                return true;
            }//the second one is in the right
            else {
                return false;
            }
    ...
    public final void swapTiles(
       ...
       // TODO - swap two tiles at the given indices
        ...
        //if(isWithinBounds(firstTileRow,firstTileCol)&&isWithinBounds(secondTileRow,secondTileCol)) {
            PuzzleGameTile temp=new PuzzleGameTile();
            temp=mTileGrid[firstTileRow][firstTileCol];
